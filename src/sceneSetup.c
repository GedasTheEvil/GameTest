...
void DrawMapItem(itmL item)
{
     ...
     glBegin(GL_QUADS);// bg
     {
	   glTexCoord2i(0, 1);   glVertex3f(0,0,0);	// Texture / Vertex Coord (Bottom Left)
	   glTexCoord2i(1, 1);   glVertex3f(0.1,0,0);	// Texutre / Vertex Coord (Bottom Right)
	   glTexCoord2i(1, 0);   glVertex3f(0.1,0.1,0);	// Texture / Vertex Coord (Top Right)
	   glTexCoord2i(0, 0);   glVertex3f(0,0.1,0);	// Texture / Vertex Coord (Top Left)
     }
     glEnd();
     glPopMatrix();
}
...
void NormalizeZ(float *A, int b)
{
	...
	for(i = 0; i < b*3 ; i+=3 ){if(A[i] < MinX)MinX = A[i]; if(A[i+1] < MinY)MinY = A[i+1]; if(A[i+2] < MinZ)MinZ = A[i+2];}

	for(i = 0; i < b*3 ; i+=3 ){A[i] -= MinX; A[i+1] -= MinY; A[i+2] -= MinZ;}
}
...
void dDungeon(int selection)
{
     ...
     {
	   if(Dun0Mobs[mobsai].alive){glLoadName(3+mobsai);DrawMon(Dun0Mobs[mobsai]);}
...
// 	   if(!selection)glCallList(DungList0);//
	...
	if(anb[0].alive){glLoadName(1);DrawMon(anb[0]);}

	if(spd[0].alive){glLoadName(2);DrawMon(spd[0]);}
